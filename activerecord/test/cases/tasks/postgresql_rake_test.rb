  ...
  module ActiveRecord
    class PostgreSQLDBCreateTest < ActiveRecord::TestCase
      def setup
        ...
        @connection_hash = @configuration.symbolize_keys.except(:adapter)
      ...
      def test_creates_database_with_default_encoding
        with_stubbed_connection_establish_connection do
          assert_called_with(
            @connection,
            :create_database,
            ["my-app-db", @connection_hash.merge(encoding: "utf8")]
          ) do
      ...
      def test_creates_database_with_given_encoding
        with_stubbed_connection_establish_connection do
          assert_called_with(
            @connection,
            :create_database,
            ["my-app-db", @connection_hash.merge(encoding: "latin")]
          ) do
            ActiveRecord::Tasks::DatabaseTasks.create @configuration.
        ...
        with_stubbed_connection do
          ActiveRecord::Base.stub(:establish_connection, nil) do
            assert_called_with(
              @connection,
              :create_database,
              ["my-app-db", @configuration.symbolize_keys.merge(encoding: "utf8").except(:adapter)]
            ) do
              ActiveRecord::Tasks::DatabaseTasks.purge @configuration
            end
          end
