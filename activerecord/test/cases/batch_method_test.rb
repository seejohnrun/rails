...
class BatchMethodTest < ActiveRecord::TestCase
  ...
  def test_preload_with_batched_methods
    ...
    scope = klass.all.preload(:interesting_number)
  ...
  def test_includes_with_batched_methods
    ...
    scope = klass.all.includes(:interesting_number)
  ...
  def test_raises_error_when_mixing_types_in_batch
    klass1 = Class.new(ActiveRecord::Base) { self.table_name = "funny_jokes" }
    klass2 = Class.new(ActiveRecord::Base) { self.table_name = "funny_jokes" }

    batch = ActiveRecord::BatchedMethods::Batch.new(klass1)

    klass1.new.batched_method_batch = batch

    assert_raises(ActiveRecord::BatchedMethods::TypeMismatch) do
