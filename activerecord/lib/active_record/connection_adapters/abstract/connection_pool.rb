...
module ActiveRecord
  ...
  module ConnectionAdapters
    ...
    class ConnectionHandler
      ...
      def establish_connection(config, owner: Base.name, role: Base.default_role_key, shard: Base.default_pool_key)
        owner = config.to_s if config.is_a?(Symbol)

        pool_config = resolve_pool_config(config, owner)
        db_config = pool_config.db_config
      ...
      def retrieve_connection(spec_name, role: ActiveRecord::Base.default_role_key, shard: ActiveRecord::Base.default_pool_key) # :nodoc:
        pool = retrieve_connection_pool(spec_name, role: role, shard: shard)

        unless pool
          if shard != ActiveRecord::Base.default_pool_key
      ...
      # already been opened.
      def connected?(spec_name, role: Base.default_role_key, shard: Base.default_pool_key)
        pool = retrieve_connection_pool(spec_name, role: role, shard: shard)
        pool && pool.connected?
      end
      ...
      def remove_connection(owner, role: Base.default_role_key, shard: Base.default_pool_key)
        remove_connection_pool(owner, role: role, shard: shard)&.configuration_hash
      end
      deprecate remove_connection: "Use #remove_connection_pool, which now returns a DatabaseConfig object instead of a Hash"

      def remove_connection_pool(owner, role: Base.default_role_key, shard: Base.default_pool_key)
      ...
      def retrieve_connection_pool(owner, role: Base.default_role_key, shard: Base.default_pool_key)
